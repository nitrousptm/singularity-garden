...
bool Scene05Bloom::init(int w, int h) {
    W = w; H = h;
    initFullscreenVAO();
    if (!shader.loadVF("shaders/common/fullscreen.vert", "shaders/scenes/scene05_bloom.frag"))
        return false;
    initialized = true;
    return true;
}

void Scene05Bloom::update(const SceneContext& ctx) {
    beatStrength = beatStrength * 0.85f + ctx.beatStrength * 0.15f;
    if (ctx.beatPhase < 0.05f) beatStrength = ctx.beatStrength;
}

void Scene05Bloom::render(const SceneContext& ctx, const Framebuffer& hdrTarget) {
    hdrTarget.bind();
    glViewport(0, 0, W, H);
